...
<h3 class="page-title uppercase bold"> <i class="fa fa-desktop"></i> Withdraw Log - Refunded</h3>
<hr>
...
//----------->>>>> PAGE
$itemPerPage = 15;
$ttl = $db->query("SELECT COUNT(*) FROM wd WHERE status='2'")->fetch();
...
$ddaa = $db->query("SELECT id, method, usr, amount, charge, tm, details, msg, status, trx FROM wd WHERE status='2' ORDER BY id DESC LIMIT ".$start.",".$itemPerPage."");
$boxtext = "Withdraw Log - Refunded";
...
<tbody>
<!-- id, method, usr, amount, charge, tm, details, msg, status, trx -->
<?php
$i = $start+1;
...
$prevnum=$page-1;
$prev ="<li> <a href=\"$adminurl/WithdrawRefunded/$prevnum\"> &lt;&lt;</a> </li>";
if($page<="1"){
...
echo "<li><a href=\"$adminurl/WithdrawRefunded/$pSt\"> $pSt</a> </li> ";
}

$pSt++;
}
$nextnum=$page+1;
$next ="<li> <a href=\"$adminurl/WithdrawRefunded/$nextnum\">&gt;&gt;</a> </li> ";
if($page>=$tpg){
