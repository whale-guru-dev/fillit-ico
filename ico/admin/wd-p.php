...
//----------->>>>> PAGE
$itemPerPage = 15;
$ttl = $db->query("SELECT COUNT(*) FROM wd WHERE status='0'")->fetch();
...
$ddaa = $db->query("SELECT id, method, usr, amount, charge, tm, details, msg, status, trx FROM wd WHERE status='0' ORDER BY id DESC LIMIT ".$start.",".$itemPerPage."");
$boxtext = "Withdraw Log - Pending";
...
$prevnum=$page-1;
$prev ="<li> <a href=\"$adminurl/WithdrawPending/$prevnum\"> &lt;&lt;</a> </li>";
if($page<="1"){
...
echo "<li><a href=\"$adminurl/WithdrawPending/$pSt\"> $pSt</a> </li> ";
}

$pSt++;
}
$nextnum=$page+1;
$next ="<li> <a href=\"$adminurl/WithdrawPending/$nextnum\">&gt;&gt;</a> </li> ";
if($page>=$tpg){
